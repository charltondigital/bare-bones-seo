...
function bare_bones_seo_render_global_map_screen() {
    if (isset($_POST['bb_save_global_map']) && check_admin_referer('bb_global_map_nonce', '_wpnonce')) {
        ...
        update_option('bare_bones_seo_global_map', $clean_settings);
        echo '<div class="updated"><p>Global Section Indexing Rules Saved Successfully.</p></div>';
    }

    $current_options = get_option('bare_bones_seo_global_map', array());
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            <?php wp_nonce_field('bb_global_map_nonce'); ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                <input type="radio" name="section_index[<?php echo esc_attr($type->name); ?>]" value="yes" <?php checked($status, 'yes'); ?> class="bb-radio-toggle">
                            </td>
                            <td style="text-align: center; vertical-align: middle;">
                                <input type="radio" name="section_index[<?php echo esc_attr($type->name); ?>]" value="no" <?php checked($status, 'no'); ?> class="bb-radio-toggle">
                            </td>
                            <td style="text-align: center; vertical-align: middle;">
                                <input type="radio" name="section_index[<?php echo esc_attr($type->name); ?>]" value="advanced" <?php checked($status, 'advanced'); ?> class="bb-radio-toggle">
                            </td>
                        </tr>
                        <tr class="bb-advanced-row-<?php echo esc_attr($type->name); ?>" style="<?php echo ($status === 'advanced') ? '' : 'display:none;'; ?> background: #fffcf0;">
                            <td colspan="4" style="padding: 12px 25px; border-left: 4px solid #f0b849;">
                                <div style="display: flex; gap: 40px; font-size: 12px;">
                                    <label><input type="checkbox" name="advanced_noindex[<?php echo esc_attr($type->name); ?>]" value="1" checked disabled> Output hard <code>&lt;noindex&gt;</code> rules on layout headers</label>
                                    <label><input type="checkbox" name="advanced_sitemap[<?php echo esc_attr($type->name); ?>]" value="1"> Force retain structural URL maps within the primary sitemap index loop</label>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <p class="submit">
                <input type="submit" name="bb_save_global_map" class="button button-primary button-large" value="Save Site Indexing Rules">
            </p>
        </form>
    </div>
    <script>
        document.querySelectorAll('.bb-radio-toggle').forEach(function(radio) {
            radio.addEventListener('change', function() {
                var rowName = this.name.match(/\[(.*?)\]/)[1];
                var advRow = document.querySelector('.bb-advanced-row-' + rowName);
