...
function render_bare_bones_redirects_tab() {
    ...
    // --- 1. HANDLE USER ACTIONS (Add / Delete) ---
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'delete_redirect' && isset($_GET['post_id']) && isset($_GET['slug'])) {
    ...
    // Handle Add Action
    if (isset($_POST['add_redirect']) && wp_verify_nonce($_POST['bb_add_redirect_nonce'], 'bb_add_redirect')) {
        ...
        if (!empty($source) && !empty($target)) {
            ...
            if ($target_post_id) {
                ...
                echo '<div class="notice notice-error"><p>Could not resolve target URL to an existing page on this site.</p></div>';
            }
        }
    }

    // --- 2. RETRIEVE & AGGREGATE REDIRECT DATA ---
    // Fetch all old slugs in one database query
    ...
    foreach ($db_results as $row) {
        ...
        if (!$target_url || is_wp_error($target_url)) {
            continue; // Skip orphan rows with no valid target page
        }
    ...
    <div class="wrap bare-bones-seo-wrap">
        ...
        <?php if (!empty($notices)) : ?>
            <?php foreach ($notices as $notice) : ?>
                <div class="notice notice-success is-dismissible" style="margin: 0 0 10px 0; border-left-color: #46b450;">
                    <p style="margin: 0; padding: 2px 0; font-size: 13px;"><?php echo $notice; ?></p>
                </div>
            <?php endforeach; ?>
            ...
            <div class="card" style="max-width: 100%; margin-bottom: 20px; padding: 16px; border: 1px solid #c3c4c7; box-shadow: none; background: #fff;">
                ...
                <form method="post" action="" style="display: flex; flex-wrap: wrap; gap: 15px; align-items: flex-end;">
                    ...
                    <div class="bb-field-group" style="margin: 0; flex: 1; min-width: 250px;">
                        ...
                        <input type="text" id="redirect_source" name="redirect_source" class="regular-text bb-input" placeholder="/old-path/" required style="height: 32px;">
                    </div>
                    ...
                    <div class="bb-field-group" style="margin: 0; flex: 1; min-width: 250px;">
                        ...
                        <input type="text" id="redirect_target" name="redirect_target" class="regular-text bb-input" placeholder="/new-destination/" required style="height: 32px;">
                    </div>
