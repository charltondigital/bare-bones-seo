...
/**
 * Catch front-end 404 errors, filter bot noise, and log/increment hits.
 */
add_action('template_redirect', 'bbseo_log_404_error');
function bbseo_log_404_error() {
    // Only log actual 404 pages on the front-end
    if (!is_404() || is_admin()) {
        return;
    }
    ...
    // BARE BONES OPTIMIZATION: Filter out common bot scans/noise to save DB space
    $ignored_patterns = array(
        ...
        'wp-admin',
        'wp-login\.php',
        '\.php$', // Often random PHP exploit probing
        '\.ashx$',
        '\.aspx$'
    );
    ...
    foreach ($ignored_patterns as $pattern) {
        if (preg_match('/' . $pattern . '/i', $request_uri)) {
            return; // Silently ignore the bot scan
        }
    }
    ...
    $referer     = isset($_SERVER['HTTP_REFERER']) ? sanitize_text_field($_SERVER['HTTP_REFERER']) : 'Direct / No Referer';
    $mysql_now   = current_time('mysql');

    // Check if we have already logged this exact 404 URL
    $existing_id = $wpdb->get_var(
        $wpdb->prepare(
            "SELECT id FROM $table_name WHERE url = %s LIMIT 1",
            $current_url
        )
    );

    if ($existing_id) {
        // Increment hits and update the timestamp
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $table_name SET hits = hits + 1, last_accessed = %s, referer = %s WHERE id = %d",
                $mysql_now,
                $referer,
                $existing_id
            )
